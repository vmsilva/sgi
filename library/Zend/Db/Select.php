...
require_once './../../../library/Zend/Db/Expr.php';
...
class Zend_Db_Select
{
    ...
    {
    	// manipiulando aspas nas variáveis join
    	//$cond = $this->leStringJoin($cond);    	
    	// fim manipulação    	
    ...
    /**
     * Manipulacao da string $cond do join, inserindo aspas
     * 
     */
    
    private function manipulaPalavraJoin($cond){
    	    	
    	if(trim($cond) == "" or strlen($cond) == 0) return $cond;
    		
    	//$uncao = array(" OR ", " AND ", " or ", " and ");
    	/*
    	$conds = array($cond);
    	foreach($uncao as $unc){
    		if(strpos($cond, $unc) !== false){
    			$c = explode(strtoupper($unc), )
    		}
    	}*/
    	
    	foreach($uncao as $unc){
    		//$cond = str_replace($unc, '"'.$unc.'"', $cond);
    	}
